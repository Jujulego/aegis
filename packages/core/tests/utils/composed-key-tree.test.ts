...
// Tests
describe('ComposedKeyTree.includes', () => {
  it('should be false if a is longer than b', () => {
    expect(ComposedKeyTree.includes(['a', 'b'], ['a'])).toBe(false);
  });

  it('should be false a as different parts than b', () => {
    expect(ComposedKeyTree.includes(['a', 'a'], ['a', 'b'])).toBe(false);
  });

  it('should be true if a is equal to b', () => {
    expect(ComposedKeyTree.includes(['a', 'b'], ['a', 'b'])).toBe(true);
  });

  it('should be true if a is a portion of b', () => {
    expect(ComposedKeyTree.includes(['a'], ['a', 'b'])).toBe(true);
  });
});
...
describe('ComposedKeyTree.search', () => {
  it('should return every item matching key', () => {
    ...
    expect(tree.search(['test'])).toEqual([11, 1, 0, 2]);
    expect(tree.search(['test', '1'])).toEqual([11, 1]);
